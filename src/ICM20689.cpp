...
int ICM20689::begin() {
  if (!this->_proSemaphore) // Create a semaphore if one does not exist
    this->_proSemaphore = xSemaphoreCreateBinary();

  xSemaphoreGive(this->_proSemaphore); // Release the semaphore
  ...
  if (writeRegister(CONFIG, GYRO_DLPF_250HZ) <
...
int ICM20689::setGyroRange(GyroRange range) {
  ...
  switch (range) {
    ...
    case GYRO_RANGE_1000DPS: {
      ...
      _gyroScale =
          1000.0f / 32767.5f * _d2r; // setting the gyro scale to 1000DPS
      break;
    }
    case GYRO_RANGE_2000DPS: {
      ...
      _gyroScale =
          2000.0f / 32767.5f * _d2r; // setting the gyro scale to 2000DPS
      break;
    }
  }
  _gyroRange = range;
...
int ICM20689::setDlpfBandwidth(DlpfBandwidth bandwidth) {
  ...
  switch (bandwidth) {
    case DLPF_BANDWIDTH_MAX: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_1046HZ) <
          0) { // setting accel bandwidth to 218Hz
        return -1;
      }
    ...
    case DLPF_BANDWIDTH_218HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_218HZ) <
          0) { // setting accel bandwidth to 218Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_250HZ) <
    ...
    case DLPF_BANDWIDTH_99HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_99HZ) <
          0) { // setting accel bandwidth to 99Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_92HZ) <
    ...
    case DLPF_BANDWIDTH_45HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_45HZ) <
          0) { // setting accel bandwidth to 45Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_41HZ) <
    ...
    case DLPF_BANDWIDTH_21HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_21HZ) <
          0) { // setting accel bandwidth to 21Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_20HZ) <
    ...
    case DLPF_BANDWIDTH_10HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_10HZ) <
          0) { // setting accel bandwidth to 10Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_10HZ) <
    ...
    case DLPF_BANDWIDTH_5HZ: {
      if (writeRegister(ACCEL_CONFIG2, ACCEL_DLPF_5HZ) <
          0) { // setting accel bandwidth to 5Hz
        return -1;
      }
      if (writeRegister(CONFIG, GYRO_DLPF_5HZ) <
...
int ICM20689::readSensor() {
  ...
  _acc[0]        = (((double) (tX[0] * _accCounts[0] + tX[1] * _accCounts[1] +
             ...
             _accB[0]) *
            _accS[0];
  _acc[1] = (((double) (tY[0] * _accCounts[0] + tY[1] * _accCounts[1] +
                        tY[2] * _accCounts[2]) *
              _accelScale) -
             _accB[1]) *
            _accS[1];
  _acc[2] = (((double) (tZ[0] * _accCounts[0] + tZ[1] * _accCounts[1] +
                        tZ[2] * _accCounts[2]) *
              _accelScale) -
             _accB[2]) *
            _accS[2];
  ...
  _gyro[0] = ((double) (tX[0] * _gyroCounts[0] + tX[1] * _gyroCounts[1] +
              ...
              _gyroScale) -
             _gyroB[0];
  _gyro[1] = ((double) (tY[0] * _gyroCounts[0] + tY[1] * _gyroCounts[1] +
                        tY[2] * _gyroCounts[2]) *
              _gyroScale) -
             _gyroB[1];
  _gyro[2] = ((double) (tZ[0] * _gyroCounts[0] + tZ[1] * _gyroCounts[1] +
                        tZ[2] * _gyroCounts[2]) *
              _gyroScale) -
             _gyroB[2];
  return 1;
...
int ICM20689_FIFO::enableFifo(bool accel, bool gyro, bool temp) {
  // use low speed SPI for register setting
  _useSPIHS = false;
  if (writeRegister(FIFO_EN,
                    (accel * FIFO_ACCEL) | (gyro * FIFO_GYRO) |
                        (temp * FIFO_TEMP)) < 0) {
    return -2;
  }
  _enFifoAccel   = accel;
...
int ICM20689_FIFO::readFifo() {
  ...
  for (size_t i = 0; i < _fifoSize / _fifoFrameSize; i++) {
    ...
    if (_enFifoAccel) {
      ...
      _axFifo[i] = (((double) (tX[0] * _accCounts[0] + tX[1] * _accCounts[1] +
                               tX[2] * _accCounts[2]) *
                     _accelScale) -
                    _accB[0]) *
                   _accS[0];
      _ayFifo[i] = (((double) (tY[0] * _accCounts[0] + tY[1] * _accCounts[1] +
                               tY[2] * _accCounts[2]) *
                     _accelScale) -
                    _accB[1]) *
                   _accS[1];
      _azFifo[i] = (((double) (tZ[0] * _accCounts[0] + tZ[1] * _accCounts[1] +
                               tZ[2] * _accCounts[2]) *
                     _accelScale) -
                    _accB[2]) *
                   _accS[2];
    ...
    if (_enFifoTemp) {
      ...
      // transform and convert to double values
      _tFifo[i] =
          ((((double) _tcounts) - _tempOffset) / _tempScale) + _tempOffset;
      _tSize = _fifoSize / _fifoFrameSize;
    }
    if (_enFifoGyro) {
      // combine into 16 bit values
      _gyroCounts[0] =
          (((int16_t) _buffer[0 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8) |
          _buffer[1 + _enFifoAccel * 6 + _enFifoTemp * 2];
      _gyroCounts[1] =
          (((int16_t) _buffer[2 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8) |
          _buffer[3 + _enFifoAccel * 6 + _enFifoTemp * 2];
      _gyroCounts[2] =
          (((int16_t) _buffer[4 + _enFifoAccel * 6 + _enFifoTemp * 2]) << 8) |
          _buffer[5 + _enFifoAccel * 6 + _enFifoTemp * 2];
      // transform and convert to double values
      _gxFifo[i] = ((double) (tX[0] * _gyroCounts[0] + tX[1] * _gyroCounts[1] +
                              tX[2] * _gyroCounts[2]) *
                    _gyroScale) -
                   _gyroB[0];
      _gyFifo[i] = ((double) (tY[0] * _gyroCounts[0] + tY[1] * _gyroCounts[1] +
                              tY[2] * _gyroCounts[2]) *
                    _gyroScale) -
                   _gyroB[1];
      _gzFifo[i] = ((double) (tZ[0] * _gyroCounts[0] + tZ[1] * _gyroCounts[1] +
                              tZ[2] * _gyroCounts[2]) *
                    _gyroScale) -
                   _gyroB[2];
...
int ICM20689::calibrateAccel() {
  ...
  for (size_t i = 0; i < _numSamples; i++) {
    readSensor();
    _accBD[0] +=
        (getAccelX_mss() / _accS[0] + _accB[0]) / ((double) _numSamples);
    _accBD[1] +=
        (getAccelY_mss() / _accS[1] + _accB[1]) / ((double) _numSamples);
    _accBD[2] +=
        (getAccelZ_mss() / _accS[2] + _accB[2]) / ((double) _numSamples);
    delay(20);
  }
  if (_accBD[0] > 9.0f) {
...
int ICM20689::writeRegister(uint8_t subAddress, uint8_t data) {
  xSemaphoreTake(this->_proSemaphore, portMAX_DELAY); // Acquire the semaphore
...
int ICM20689::readRegisters(uint8_t subAddress, uint8_t count, uint8_t *dest) {
  ...
  if (_useSPI) {
    ...
    _spi->transfer(subAddress |
                   SPI_READ); // specify the starting register address
